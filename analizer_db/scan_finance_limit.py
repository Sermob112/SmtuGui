...
# Ключевые слова для поиска (лимит финансирования)
# SEARCH_KEYWORDS = [
#     "лимит",
#     "финансирование",
# "финансирования",
...
def main():
    ...
    query = f"""
        SELECT Id, RegistryNumber, PurchaseName, {fields_sql}
        FROM purchase
        WHERE 
            { " OR ".join([f"{field} IS NOT NULL AND {field} != ''" for field in JSON_FIELDS]) }
    """
    ...
    for row in rows:
        ...
        purchase_name = row[2]
        json_values = row[3:]  # common_info_json, lots_json, protocols_json, contracts_info_json
        ...
        if row_matches:
            ...
            print(f"PurchaseName: {purchase_name}")
