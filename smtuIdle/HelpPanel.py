...
"""
        ...
        self.SecSys = QPushButton("Техническая документация")
        ...
        self.Qword = QLabel("Руководство пользователя")
        
        menu_layout.addWidget(self.Qword)
        self.files = os.listdir("HelpFiles")
    
        if self.files[0].endswith(".docx"):
            button = QPushButton(self.files[0])
            button.setFixedSize(400, 30)
            button.clicked.connect(lambda checked: self.open_file(self.files[0]))
        ...
        if self.files[1].endswith(".docx"):
            button = QPushButton(self.files[1])
            button.setFixedSize(400, 30)
            button.clicked.connect(lambda checked:  self.open_file(self.files[1]))
